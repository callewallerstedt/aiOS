...
export class Transcript {
  ...
  ensureToolRun() {
    ...
    const node = this.addRow("tool-run", `
      <button type="button" class="tool-run-head" aria-expanded="false">
        ...
        <span class="tool-run-summary">1 tool call</span>
        <span class="tool-run-latest" hidden><span class="tool-run-live-dot" aria-hidden="true"></span><span class="tool-run-latest-text"></span></span>
      </button>
    ...
    `);
    this.toolRun = { node, body: node.querySelector(".tool-run-rows"), cards: new Set(), fileDiffs: new Map(), sequence: 0 };
    return this.toolRun;
  }
  ...
  updateToolRun(run) {
    ...
    const active = cards.filter((card) => ["started", "update"].includes(String(card.state.phase || "")))
      .sort((a, b) => Number(b.updatedOrder || 0) - Number(a.updatedOrder || 0))[0];
    const latest = active || cards.sort((a, b) => Number(b.updatedOrder || 0) - Number(a.updatedOrder || 0))[0];
    ...
    diffs.innerHTML = rows.map((row) => {
      const diff = String(row.diff || "").trim();
      const diffHtml = diff
        ? `<pre class="tool-run-diff-body">${escapeHtml(diff)}</pre>`
        : "";
      return `
      <button type="button" class="tool-run-diff" data-reveal="${escapeHtml(row.path)}" title="${escapeHtml(row.path)}">
        <span class="tool-run-file">${escapeHtml(row.name)}</span>
        <span class="add">+${Number(row.add || 0).toLocaleString()}</span>
        ${Number(row.del || 0) ? `<span class="del">-${Number(row.del).toLocaleString()}</span>` : ""}
      </button>
      ${diffHtml}
    `;
    }).join("");
  ...
  finishTurnDiffs() {
    ...
    this.addRow("turn-diffs", `
      ...
      ${rows.map((row) => `
        <button type="button" class="turn-diff" data-reveal="${escapeHtml(row.path)}" title="${escapeHtml(row.path)}">
          <span class="turn-diff-file">${escapeHtml(row.name)}</span>
          <span class="add">+${Number(row.add || 0).toLocaleString()}</span>
          ${Number(row.del || 0) ? `<span class="del">-${Number(row.del || 0).toLocaleString()}</span>` : ""}
        </button>
      `).join("")}
    `);
  }
  ...
  streamThinking(activity, raw = {}) {
    ...
    if (!this.thinkingEl || this.thinkingKey !== key) {
      this.thinkingKey = key;
      this.thinkingText = "";
      this.thinkingStartedAt = Date.now();
      this.thinkingEl = this.addRow("thinking expanded", `
        <button type="button" class="thinking-head" aria-expanded="true">
          <span class="thinking-star" aria-hidden="true">${SPARK_ICON}</span>
          <span class="thinking-label">Thinking</span>
          <span class="thinking-time"></span>
          <span class="thinking-chevron" aria-hidden="true">${CHEVRON_ICON}</span>
        </button>
        <div class="thinking-expand">
          <div class="thinking-inner">
            <span class="thinking-track" aria-hidden="true"></span>
            <div class="thinking-trace-row">
    ...
    if (raw.delta) this.thinkingText += raw.delta;
    else if (activity.summary) this.thinkingText = String(activity.summary);

    const phase = String(raw.phase || activity.phase || "").toLowerCase();
    const done = phase === "completed" || phase === "failed";
    // Shimmer while the model is still thinking, settle when it lands.
    this.thinkingEl.classList.toggle("live", !done);

    const body = this.thinkingText.trim();
    this.thinkingEl.querySelector(".thinking-content").innerHTML = body
      ? renderMarkdown(body)
      : renderMarkdown(String(activity.title || "Thinking"));
    const elapsed = Math.max(0, Number(activity.duration_ms || raw.duration_ms || 0) / 1000)
      || Math.max(0, (Date.now() - this.thinkingStartedAt) / 1000);
    this.thinkingEl.querySelector(".thinking-label").textContent = done
      ? `Thought for ${formatDuration(elapsed)}`
      : "Thinking";
    this.thinkingEl.querySelector(".thinking-time").textContent = done ? "" : formatDuration(elapsed);
    if (done) this.closeThinking();
  }

  closeThinking() {
    if (this.thinkingEl) {
      this.thinkingEl.classList.remove("live");
      if (this.thinkingEl.dataset.manual !== "true") this.thinkingEl.classList.remove("expanded");
      this.thinkingEl.querySelector(".thinking-head")
        ?.setAttribute("aria-expanded", String(this.thinkingEl.classList.contains("expanded")));
    }
    this.thinkingEl = null;
    this.thinkingKey = null;
    this.thinkingText = "";
  ...
  paintActivity(card) {
    ...
    // Plans are always expanded: the checklist is the whole point of the card.
    if (type === "plan") node.classList.add("expanded");
    ...
    if (glyph.dataset.mode !== glyphMode) {
      // Plans have no glyph at all -- just the "Plan" label. The active step
      // carries the churning animation instead.
    ...
    node.querySelector(".name").textContent = type === "plan" ? "Plan" : visibleName;
    ...
    node.querySelector(".agent-status").textContent = type === "plan"
      ? `${done}/${steps.length}`
      : busy ? "Running" : state.phase === "failed" ? "Failed" : "Done";
    node.classList.toggle("plan-complete", type === "plan" && steps.length > 0 && done === steps.length);
    node.querySelector(".steps").innerHTML = steps.map((step) => {
      const status = String(step.status || "pending");
      const mark = status === "completed"
        ? '<span class="mark">\u2713</span>'
        : status === "in_progress"
          ? `<span class="mark">${loadingPixelsMarkup()}</span>`
          : '<span class="mark"></span>';
      return `<div class="step ${escapeHtml(status)}">${mark}${escapeHtml(String(step.step || ""))}</div>`;
    }).join("");
